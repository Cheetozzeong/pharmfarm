...
$supervisorTree = [Microsoft.CodeAnalysis.CSharp.CSharpSyntaxTree]::ParseText([IO.File]::ReadAllText((Join-Path $RepositoryRoot 'windows-agent-production/PharmFarm-AgentSupervisor.cs')))
$references = [Microsoft.CodeAnalysis.MetadataReference[]]@(
  ...
  [Microsoft.CodeAnalysis.MetadataReference]::CreateFromFile((Join-Path $ReferenceDirectory 'System.Management.dll'))
)
...
$compilation = [Microsoft.CodeAnalysis.CSharp.CSharpCompilation]::Create('PharmFarm-AgentHost', [Microsoft.CodeAnalysis.SyntaxTree[]]@($tree, $supervisorTree), $references, $options)
